...
structured_book_agent = Agent(
    name="structured_book_agent",
...
root_agent=Agent(
    ...
    2. 수집 된 정보를 structured_book_agent 에이전트에 전달하여 BookList 형식으로 구조화된 추천 목록을 생성합니다.
    3. 최종 결과를 사용자에게 전달합니다.
    각 에이전트의 역할:
    - book_data_finder : IT 도서 베스트셀러 정보 조회
    - structured_book_agent: 구조화된 BookList 형식으로 출력 생성
    """,
    sub_agents=[book_data_agent, structured_book_agent], #하위 에이전트들 
